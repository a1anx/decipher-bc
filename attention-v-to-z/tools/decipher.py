...
def predictive_log_likelihood(decipher, dataloader, n_samples=5):
    ...
    # NEW: check once whether batch correction is active so we can unpack xc correctly
    use_bc = decipher.config.use_batch_correction
    try:
        for i in range(n_samples):
            total_log_prob = 0
            for xc in dataloader:
                xc = [x.to(decipher.device) for x in xc]
                # NEW: when batch correction is on, xc = [x, (context,) batch_index].
                # Unpack explicitly so model and guide both receive batch_index.
                # OLD: guide_trace = poutine.trace(decipher.guide).get_trace(*xc)
                if use_bc:
                    x = xc[0]
                    context = xc[1] if len(xc) > 2 else None
                    batch_index = xc[-1]
                    args = (x, context, batch_index)
                else:
                    args = tuple(xc)
                guide_trace = poutine.trace(decipher.guide).get_trace(*args)
...
def decipher_train(
    ...
    plot_kwargs=None,
    device="cpu",
    batch_key=None,  # NEW PARAM: column in adata.obs with batch labels; enables batch correction
):
    ...
    batch_key: str, optional
        Column in `adata.obs` containing batch labels. When provided, enables
        batch correction via cross-attention at the v->z step. Default: None.
    ...
    # NEW: pass batch_key so config counts n_batches and sets use_batch_correction = True
    # OLD: decipher_config.initialize_from_adata(adata_train)
    decipher_config.initialize_from_adata(adata_train, batch_key=batch_key)

    # NEW: pass batch_index_key so each DataLoader batch includes the integer batch index tensor
    # OLD: dataloader_train = make_data_loader_from_adata(adata_train, decipher_config.batch_size, drop_last=True)
    dataloader_train = make_data_loader_from_adata(
        adata_train, decipher_config.batch_size, batch_index_key=batch_key, drop_last=True
    )
    # OLD: dataloader_val = make_data_loader_from_adata(adata_val, decipher_config.batch_size)
    dataloader_val = make_data_loader_from_adata(adata_val, decipher_config.batch_size, batch_index_key=batch_key)
    ...
    val_nll = np.nan
    # NEW: cache flag so we don't re-check config on every batch
    use_bc = decipher_config.use_batch_correction
    for epoch in pbar:
        train_losses = []
        decipher.train()
        if epoch > 0:
        ...
        for xc in dataloader_train:
            try:
                ...
                # NEW: when batch correction is on, unpack x, context, and batch_index explicitly
                # so the model receives batch_index for the v->z attention step.
                # OLD: loss = svi.step(*xc)
                if use_bc:
                    x = xc[0]
                    context = xc[1] if len(xc) > 2 else None
                    batch_index = xc[-1]
                    loss = svi.step(x, context, batch_index)
                else:
                    loss = svi.step(*xc)
        ...
        if plot_every_k_epochs > 0 and (epoch % plot_every_k_epochs == 0):
            ...
            if is_notebook():
                # OLD: from IPython.core import display / display.clear_output — moved in newer IPython
...
def decipher_gene_imputation(adata):
    ...
    """
    decipher = decipher_load_model(adata)
    # OLD: imputed = decipher.impute_gene_expression_numpy(adata.X.toarray())
    # NEW: use get_dense_X to safely handle both sparse and dense .X formats.
    # No batch_index needed here: the z->x decoder is not batch-corrected in this model.
    imputed = decipher.impute_gene_expression_numpy(get_dense_X(adata))
