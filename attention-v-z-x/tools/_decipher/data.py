...
# NEW PARAM: batch_index_key — when provided, appends a LongTensor of batch codes to the dataset
def make_data_loader_from_adata(adata, batch_size=64, context_discrete_keys=None, batch_index_key=None, **kwargs):
    ...
    # NEW: append the integer batch index tensor so the training loop can pass it to the model
    if batch_index_key is not None:
        batch_index = torch.LongTensor(
            adata.obs[batch_index_key].astype("category").cat.codes.values
        )
