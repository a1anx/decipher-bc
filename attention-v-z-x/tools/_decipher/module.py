...
# NEW CLASS: analogous to BatchCorrectedDecoder (in attention-z-to-x), but placed at the
# v -> z step instead. v is the query, all batch prototype embeddings E_b are keys/values,
# and the output is (z_loc, z_scale) rather than a single logit vector.
class BatchCorrectedVToZ(nn.Module):
    """Decoder where v attends over all n_batches prototype embeddings via cross-attention
    to produce batch-aware z parameters (z_loc, z_scale).

    query: cell's projected v,           shape (1,         batch_size, batch_emb_dim)
    key/value: all n_batches prototypes, shape (n_batches, batch_size, batch_emb_dim)

    Softmax is over n_batches keys, so v drives the attention weights.
    """
    ...
    def __init__(
        ...
        # Project v into the attention embedding space if dimensions differ
        if dim_v != batch_emb_dim:
            self.v_projection = nn.Linear(dim_v, batch_emb_dim)
        else:
        ...
        # MLP input is [v, attention_output] concatenated (or summed in "add" mode)
        if combination_mode == "concat":
            mlp_input_dim = dim_v + batch_emb_dim
        elif combination_mode == "add":
            if dim_v != batch_emb_dim:
                raise ValueError(
                    f"'add' mode requires dim_v == batch_emb_dim, "
                    f"got {dim_v} vs {batch_emb_dim}"
                )
            mlp_input_dim = dim_v
        else:
            raise ValueError(f"Unknown combination_mode: {combination_mode}")

        # Output is 2 * dim_z so we can split into z_loc and z_scale
        layers = []
        ...
        for hidden_dim in hidden_dims:
            ...
            prev_dim = hidden_dim
        layers.append(nn.Linear(prev_dim, 2 * dim_z))
    ...
    def forward(self, v, _batch_index=None):
        """
        Args:
            v:             (batch_size, dim_v)
            _batch_index:  unused — all batch prototypes are attended over, not just
                           the cell's own batch. Kept for API consistency with calling code.
        Returns:
            z_loc:   (batch_size, dim_z)
            z_scale: (batch_size, dim_z)  (raw, before softplus)
        """
        batch_size = v.shape[0]

        v_proj = self.v_projection(v)                                        # (batch_size, batch_emb_dim)

        all_batch_embs = self.batch_embedding.weight                         # (n_batches, batch_emb_dim)
        query = v_proj.unsqueeze(0)                                          # (1, batch_size, batch_emb_dim)
        ...
        if self.combination_mode == "concat":
            ...
            combined = v + attn_output

        out = self.mlp(combined)                                             # (batch_size, 2 * dim_z)
        z_loc   = out[..., :self.dim_z]                                      # (batch_size, dim_z)
        z_scale = out[..., self.dim_z:]                                      # (batch_size, dim_z)
        return z_loc, z_scale

    def get_attention_weights(self, v):
        """
        Returns:
...
# NEW CLASS: copied from attention-z-to-x/module.py.
# z attends over all n_batches prototype embeddings to produce a batch-corrected gene expression mean.
# Used at the z->x step to ensure x reconstruction is also batch-aware.
class BatchCorrectedDecoder(nn.Module):
    ...
    def __init__(
        ...
        # Project z into the attention embedding space if dimensions differ
        ...
        # MLP input is [z, attention_output] concatenated (or summed in "add" mode)
        if combination_mode == "concat":
    ...
    def forward(self, z, _batch_index=None):
        """
        Args:
