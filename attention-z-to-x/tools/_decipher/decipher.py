...
class DecipherConfig:
    ...
    def initialize_from_adata(self, adata, batch_key = None): #EDIT: Add batch_key = NONE
        self.dim_genes = adata.shape[1]
        self.n_cells = adata.shape[0]
        if batch_key is not None: #EDIT: IF STATEMENT
...
class Decipher(nn.Module):
    ...
    def __init__(
        ...
        #EDIT: NEW PATH FOR BATCH CORRECTION
        ...
        else: #EDIT put self.decoder_z_to_x over else statement 
    ...
    def model(self, x, context=None, batch_index=None): #EDIT: Add batch_index=None
    ...
    def guide(self, x, context=None,batch_index=None): #EDIT: Add batch_index=None
        pyro.module("decipher", self)
        with pyro.plate("batch", len(x)), poutine.scale(scale=1.0):
            x = torch.log1p(x)
    ...
    def impute_gene_expression_numpy(self, x, batch_index=None): #EDIT: Add batch_index=None
        if type(x) == np.ndarray:
            x = torch.tensor(x, dtype=torch.float32)
