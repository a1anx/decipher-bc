    ...
    try:
        for i in range(n_samples):
            ...
            for xc in dataloader:
                ...
                #EDIT: IF Else statement for use_bc
                if use_bc:
                    x = xc[0]
                ...
                ).get_trace(*args) #EDIT: Was previously ).get_trace(*xc)
...
def decipher_train(
    ...
    plot_kwargs=None,
    device="cpu",
    batch_key=None #EDIT: Add batch_key=None
):
    ...
    batch_key: str, optional
        ...
        batch correction via cross-attention over all batch prototype embeddings.
        Default: None (no batch correction).   
    ...
    decipher_config.initialize_from_adata(adata_train, batch_key=batch_key) #EDIT: ADD batch_key=batch_key

    dataloader_train = make_data_loader_from_adata(
        adata_train, decipher_config.batch_size, batch_index_key=batch_key,drop_last=True #EDIT: Add batch_index_key=batch_key
    )
    ...
    for epoch in pbar:
        ...
        if plot_every_k_epochs > 0 and (epoch % plot_every_k_epochs == 0):
            ...
            if is_notebook():
                """
                EDIT: clear_output was moved from IPython.core.display to Ipython.display. Old location doesn't have it anymore
    ...
    if is_notebook():
        """
        OLD: clear_output was moved from IPython.core.display to Ipython.display
        from IPython.core import display
...
def decipher_gene_imputation(adata, batch_key=None): #EDIT: Added batch_key=None
    ...
    adata: sc.AnnData
        The annotated data matrix.
    batch_key: str, optional
        Column in `adata.obs` containing batch labels. Required when the model
        was trained with batch correction. Default: None.
    ...
    if batch_key is not None:
        ...
        imputed = decipher.impute_gene_expression_numpy(x, batch_index=batch_index)
    else:
        imputed = decipher.impute_gene_expression_numpy(x)
    #END EDIT:
