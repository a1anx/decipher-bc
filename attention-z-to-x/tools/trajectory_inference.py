...
def gene_patterns(adata, l_scale=10_000, n_samples=100, reference_batch=0): #ADDED: reference_batch=0
    ...
    reference_batch : int, optional
        When the model was trained with batch correction, decode using this batch prototype index. Default: 0.
    ...
    use_bc = model.config.use_batch_correction
    ...
    for t_name in adata.uns["decipher"]["trajectories"]:
        ...
        #NEW COMMENTS: 
        # z_mean:    (n_points, dim_z)
        # z_samples: (n_samples, n_points, dim_z)
        gene_patterns = dict()
        """
        OLD: 
        gene_patterns["mean"] = (
        ...
        #NEW EDIT: 
        if use_bc:
            n_points = z_mean.shape[0]
            ref_idx = torch.full((n_points,), reference_batch, dtype=torch.long)

            gene_patterns["mean"] = (
                torch.nn.functional.softmax(model.decoder_z_to_x(z_mean, ref_idx), dim=-1)
                .detach().numpy()
                * l_scale
            )

            # Flatten (n_samples, n_points, dim_z) → (n_samples*n_points, dim_z), decode, reshape back
            z_flat = z_samples.view(-1, z_samples.shape[-1])
            ref_idx_flat = torch.full((z_flat.shape[0],), reference_batch, dtype=torch.long)
            gene_expression_samples = (
                torch.nn.functional.softmax(model.decoder_z_to_x(z_flat, ref_idx_flat), dim=-1)
                .view(n_samples, n_points, -1)
                .detach().numpy()
                * l_scale
            )
        else:
            gene_patterns["mean"] = (
                torch.nn.functional.softmax(model.decoder_z_to_x(z_mean), dim=-1)
                .detach().numpy()
                * l_scale
            )
            gene_expression_samples = (
                torch.nn.functional.softmax(model.decoder_z_to_x(z_samples), dim=-1)
                .detach().numpy()
                * l_scale
            )
        #END EDIT
