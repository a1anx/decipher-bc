...
from decipher_models.tools.diagnostics import (
    reconstruction_r2_log1p as _tl_reconstruction_r2_log1p,
)
...
def reconstruction_r2_log1p(
    ...
    if (
        recompute
        or cached_adata_id != id(adata)
        or not hasattr(decipher, "reconstruction_r2_result_")
    ):
    ...
    idx = rng.choice(flat_x.size, size=min(n_scatter_points, flat_x.size), replace=False)
