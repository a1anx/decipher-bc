...
class DecipherConfig:
    ...
    batch_embedding_mode: Literal["concat", "additive"] = "concat"
    n_batches: int = 0
...
class Decipher(nn.Module):
    ...
    def __init__(
        ...
        # ---- batch conditioning ----
        if config.batch_conditioning != "none":
            if config.batch_embedding_mode == "concat":
        ...
        encoder_extra = (
            self.config.dim_batch_embedding
            if (
                config.batch_conditioning == "decoder_encoder"
                and config.batch_embedding_mode == "concat"
            )
            else 0
        )
        decoder_extra = (
            self.config.dim_batch_embedding
            if (config.batch_conditioning != "none" and config.batch_embedding_mode == "concat")
            else 0
        )

        # 2. Encoder
        self.encoder_x_to_z = ConditionalDenseNN(
            self.config.dim_genes + encoder_extra, [128], [self.config.dim_z] * 2
        )
        ...
        ## z -> x (reconstruction) -- never batch-conditioned
    ...
    def model(self, x, batch_idx=None):
        ...
        if bc != "none":
            ...
            if self.config.batch_embedding_mode == "concat":
        ...
        with pyro.plate("batch", len(x)), poutine.scale(scale=1.0):
            ...
            # v -> z prior
            if bc != "none" and self.config.batch_embedding_mode == "concat":
            ...
            # z -> x reconstruction, never conditioned on batch
    ...
    def compute_v_z_numpy(self, x: np.array, batch_idx=None):
        ...
        batch_idx : torch.Tensor, optional
            Integer batch codes of shape (n_cells,). Only used when
            `batch_conditioning == "decoder_encoder"`; defaults to batch 0 for all cells.
        ...
        if bc == "decoder_encoder":
            ...
            if self.config.batch_embedding_mode == "concat":
                batch_vec = self.batch_emb(batch_idx)
                z_loc, _ = self.encoder_x_to_z(torch.cat([x, batch_vec], dim=-1))
            else:
