...
from decipher_vz_add.utils import DECIPHER_GLOBALS, create_decipher_uns_key
...
def get_batch_idx(adata, config):
    ...
    ADDED 2026-08-24 -- needed by fix 5 (batch_shift_review_handoff.md). This arm does
    not need fix 1, because its guide is batch-blind and its imputation was already
    correct. It still needs the codes at export time: decipher_z is defined as
    `encoder_x_to_z(x) - batch_shift[b] + mean(batch_shift)`, and that needs each cell's
    own b. Keeping the conversion in one place is what stops the training and export
