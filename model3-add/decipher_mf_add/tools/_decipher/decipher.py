...
from decipher_mf_add.tools._decipher.module import ConditionalDenseNN
...
class Decipher(nn.Module):
    ...
    def __init__(
        ...
        #    exactly, on every sample, out of the z log-density ratio. In this mean-field
        #    variant that left batch_shift trained by reconstruction alone, since
        #    encoder_x_to_v does not take z and so carries no second gradient path.
        #  fix 3 -- nn.Embedding defaults to Normal(0,1), measured per-dim std ~1.02,
        #    against a true per-batch shift std of 0.058-0.173. Zero is the standard
        #    start for an intercept, and at zero the z prior is exactly base Decipher's.
        ...
        # 2. Encoder (z-branch is batch-blind; batch shift added to z_loc after)
    ...
    def compute_v_z_numpy(self, x: np.array, batch_idx=None):
        ...
        # WAS: v_loc, _ = self.encoder_zx_to_v(x)  # mean field: never touched batch or z
        v_loc, _ = self.encoder_x_to_v(x)  # fix 7: renamed; mean field, counts only
