...
from decipher_zx.plot.decipher import decipher as plot_decipher_v
from decipher_zx.tools._decipher import Decipher, DecipherConfig
from decipher_zx.tools._decipher.data import (
    decipher_load_model,
    decipher_save_model,
    make_data_loader_from_adata,
    get_batch_idx,  # added 2026-08-24 (fix 5)
    get_dense_X,
)
# Stale references to the old `decipher` package name; decipher_zx has its own copies of these.
...
from decipher_zx.tools.utils import EarlyStopping
from decipher_zx.utils import DECIPHER_GLOBALS, GIFMaker, is_notebook, load_and_show_gif
...
def decipher_train(
    ...
    # fix 4 (2026-08-24): exclude the batch-shift table from weight decay.
    #
    ...
    # pyro.module("decipher", self) registers the shift table. That is a shrink-to-zero
    ...
    # module prefix (e.g. "decipher.batch_shift.weight"), so this has to be a substring
    # test rather than an equality check.
    def _per_param_optim_args(param_name):
        # NOTE 2026-08-25 -- this substring test is now DEAD in this arm. B6/B7 removed
        # every batch_shift table, so no parameter name contains "batch_shift" and every
        # parameter receives weight_decay=1e-4.
        #
        # That is a real behaviour change, NOT covered by any Part 7 decision. Fix 4
        # existed because weight decay on a per-batch INTERCEPT is an unmotivated
        # shrink-to-zero prior, and §2.3 showed it broke the identifiability tie toward
        # "batch_shift = 0, v carries the batch". The batch parameters are now ordinary
        # columns of decoder_z_to_x's and encoder_x_to_z's Linear weights, so they are
        # decayed like every other weight in the network.
        #
        # Arguably correct -- they are network weights now, and the flat direction fix 4
        # was protecting no longer exists, since B6 deleted the p(z|v,b) edge that created
        # it. Left as-is deliberately rather than silently reinstated. Flagged for Alan; if
        # the batch columns should be exempt, the test becomes a check on the first
        # n_batches columns of those two weights, which per-parameter args cannot express
        # (they apply to whole tensors) and would need a parameter-group split instead.
        args = {"lr": decipher_config.learning_rate, "weight_decay": 1e-4}
        if "batch_shift" in param_name:
            args["weight_decay"] = 0.0  # per-batch intercept: no shrinkage (dead: see above)
        return args
...
def decipher_gene_imputation(adata):
    ...
    """
    decipher = decipher_load_model(adata)
    # ADDED 2026-08-25 -- B5: reconstruct each cell through its own batch, not batch 0.
...
def _decipher_to_adata(decipher, adata):
    ...
    #
    # This arm used to export encoder_x_to_z(x) with no shift term -- the UNCORRECTED
    # coordinate -- while models 2/3 exported the corrected one under the same column
    # name. Per §2.2 model 1 and model 2 are the same model written differently, so the
    # two arms were reporting opposite quantities and decipher_z was not comparable
    # across them. Both are now written, defined identically in all three arms.
    #
    # This DOES change this arm's decipher_z, and therefore its Leiden clusters and rho.
    batch_idx = get_batch_idx(adata, decipher.config)
