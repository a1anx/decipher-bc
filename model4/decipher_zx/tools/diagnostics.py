...
from decipher_zx.tools._decipher.data import get_dense_X, get_batch_idx
...
def reconstruction_r2_log1p(decipher, adata):
    ...
    # ADDED 2026-08-25 -- B5 brings finding 1 to this arm. model1-add did NOT need this
    # because its decoder_z_to_x was batch-blind, so reconstruction did not depend on b at
    # all. decoder_z_to_x now takes b, so without the real codes every cell would be
    # reconstructed through batch 0's decoder columns -- wrong for n_batches-1 of the
    # batches, and biased downward, since the wrong batch offset can only add error.
...
def batch_effect_recovery(decipher, adata, batch_key="batch"):
    """Compare the batch effect the model LEARNED against the one in the data.

    ADDED 2026-08-25. Replaces the diagnostic that `decipher_z_raw - decipher_z` used to
    provide, which stopped working when B7 replaced the additive batch table with a
    concatenated one-hot (handoff Part 7, B9).
    ...
    uns["gene_shift_matrix"]? Two mismatches stack, and neither is a bug:
      ...
      1. Softmax shift-invariance. mu = softmax(decoder_z_to_x(...)), so adding a constant
         to all n_genes logits of one batch changes nothing. Each batch column is
         identified only up to an additive constant, so a raw correlation against the truth
         is not well defined.
      2. Different spaces. The decoder's offset is additive in PRE-SOFTMAX LOGITS, which
         after softmax and the library-size multiply is multiplicative per gene. The
         simulator's gene_shift is additive in `pre_x`, before round / min-subtract / clip.

    Both go away by comparing all the way downstream, in the space the counts actually
    live in: per-batch mean log1p expression, centred across batches. That is the same
    quantity on both sides, so the comparison is well posed no matter which functional
    form produced it.
    ...
    Both matrices are centred across batches, which removes the softmax constant and makes
    the comparison invariant to the gauge freedoms described above.
    """
    x = get_dense_X(adata)
    batch_idx = get_batch_idx(adata, decipher.config)
